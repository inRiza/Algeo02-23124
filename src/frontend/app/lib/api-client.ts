import { AudioMatch, ImageMatch, ImageFile, QueryResponse } from './types';
...
export const audioApi = {
    async uploadQuery(file: File): Promise<{matches: AudioMatch[], executionTime: number}> {
        ...
        if (!response.ok) throw new Error('Failed to upload query');
...
export const imageApi = {
    async uploadQuery(file: File): Promise<QueryResponse> {
        const formData = new FormData();
        formData.append('file', file);
    ...
    async getDataset(): Promise<ImageFile[]> {
        const response = await fetch(`${API_BASE_URL}/image/dataset`);
        if (!response.ok) throw new Error('Failed to fetch dataset');
        return response.json();
    },
